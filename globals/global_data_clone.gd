	...
	if dir:
		...
		while file_name != "":
			if file_name.ends_with(".zip"):
				...
				if !fl.has("vanilla") and !fl.has("classes") and !fl.has("globals") and fl.size() == 1: ProjectSettings.load_resource_pack(full_pck_path, true);
...
func load_mods() -> void:
	...
	commands_containers.clear();
	
	for container in commands_container_for_commands_containers.get_children(): container.queue_free();
	...
	for mod_name in main_folders:
		...
		if ResourceLoader.exists(path + "data.gd"):
			...
			if "mod_info" in mod_data_file:
				var info : Dictionary = mod_data_file.mod_info;
				...
				mods_list[mod_name] = mod_data_file.mod_info;
				
			else: mods_list[mod_name] = fallback_info;
			if "mod_data" in mod_data_file: data[mod_name] = mod_data_file.mod_data;
		else:
			var _fallback_info : Dictionary = {mod_name : fallback_info};
			mods_list.merge(_fallback_info);
...
func execute_command(sender : Entity = null, _name : String = "give_item", mod_name : String = "vanilla", args : Array = []) -> Error:
	...
	if command.is_valid(): command.callv(args);
	return OK;
