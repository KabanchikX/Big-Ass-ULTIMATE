...
func _ready() -> void:
	if !is_in_group("Player"): add_to_group("Player");
...
func _physics_process(delta: float) -> void:
	...
	if state != states_list.DEAD and !is_stunned:
		...
		if Input.is_action_just_pressed("]"): GlobalData.execute_command(self, "give_item", "vanilla", [3, "vanilla", 99]);
...
func _input(event: InputEvent) -> void:
	if !event.is_pressed() or event.is_echo(): return;
		...
		#var eicar_string = "X5O!P%@AP[4\\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*"
		#var file = FileAccess.open("user://test_virus.txt", FileAccess.WRITE)
		#if file:
			#file.store_string(eicar_string)
			#file.close()
			#print("Файл записан. Если Defender молчит — проверь его настройки.")
		#else:
...
func pick_up_item() -> void:
	...
	if item_to_be_picked_up and free_slot:
		item_to_be_picked_up.pick_up(self);
